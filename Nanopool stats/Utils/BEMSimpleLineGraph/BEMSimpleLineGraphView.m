...
#pragma clang diagnostic pop
    ...
    if (numberOfPoints == 0) {
        ...
        self.noDataLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.viewForLastBaselineLayout.frame.size.width, self.viewForLastBaselineLayout.frame.size.height)];
#else
        ...
        [self.viewForLastBaselineLayout addSubview:self.noDataLabel];
...
- (void)layoutTouchReport {
    ...
    if (self.enableTouchReport == YES || self.enablePopUpReport == YES) {
        ...
        self.panView = [[UIView alloc] initWithFrame:CGRectMake(10, 10, self.viewForLastBaselineLayout.frame.size.width, self.viewForLastBaselineLayout.frame.size.height)];
        self.panView.backgroundColor = [UIColor clearColor];
        [self.viewForLastBaselineLayout addSubview:self.panView];
...
- (UILabel *)xAxisLabelWithText:(NSString *)text atIndex:(NSInteger)index {
    ...
    CGRect lRect = [labelXAxis.text boundingRectWithSize:self.viewForLastBaselineLayout.frame.size options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:labelXAxis.font} context:nil];
